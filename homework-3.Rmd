...
cbind(
  lmm_model %>% summary %>% coef %>% data.frame %>%
    select(GEE.indp = Estimate, SE = Std..Error),
...
Table 1 shows coefficient estimates and corresponding standard errors for linear mixed-effects model (LMM) and generalized estimating equation (GEE) models with independent and exchangeable working correlations.
...
# read in data
seizure_wide <- read.csv("data/Seizure.csv")
# seizure_wide %>% head
...
seizure_long <- seizure_wide %>%
  pivot_longer(cols = starts_with("y"),
                      ...
                      names_prefix = "y",
                      values_to = "seizures") %>%
  # redefine biweek to week (values from 0,1,2,3,4 to 0,2,4,6,8)
  mutate(week = 2L * as.integer(biweek)) %>% 
  select(-X, -biweek)
# seizure_long %>% head
```
